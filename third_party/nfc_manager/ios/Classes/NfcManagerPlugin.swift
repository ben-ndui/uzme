...
extension NfcManagerPlugin: NFCTagReaderSessionDelegate {
  ...
  public func tagReaderSession(_ session: NFCTagReaderSession, didInvalidateWithError error: Error) {
    // UZME PATCH #1 : la session invalidée (feuille fermée par
    // l'utilisateur, timeout 60s) doit libérer la référence native —
    // sans ça, tout begin suivant lève session_already_exists jusqu'au
    // kill de l'app.
    tagSession = nil
...
private func convert(_ value: NFCNDEFTag, _ completionHandler: @escaping (TagPigeon?, Error?) -> Void) {
  ...
  value.queryNDEFStatus { status, capacity, error in
    ...
    value.readNDEF { message, error in
      if let error = error {
        // UZME PATCH #2 : un tag NDEF-formaté mais VIDE (NTAG21x sorti
        // d'usine) fait retourner "zero length message" par readNDEF.
        // Sans ce cas, la détection est silencieusement abandonnée et
        // les tags vierges sont invisibles pour l'app — traiter comme
        // "pas de message en cache" et livrer le tag quand même.
        let isZeroLength =
